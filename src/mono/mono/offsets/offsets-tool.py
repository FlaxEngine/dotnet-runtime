...
class OffsetsTool:
	...
	def parse_args(self):
		...
		# Switch
			...
			require_sysroot (args)
			self.target = Target ("TARGET_ARM64", "TARGET_SWITCH", ["TARGET_SWITCH", "HOST_SWITCH", "MONO_CROSS_COMPILE", "USE_MONO_CTX", "NN_NINTENDO_SDK", "NN_SDK_BUILD_RELEASE", "__NINTENDO_OS__"])
			self.target_args += ["--target=aarch64-nintendo-nx-elf", "-Wno-macro-redefined"]
			self.sys_includes = [args.sysroot + "/Include", args.sysroot + "/TargetSpecificInclude/NX-NXFP2-a64", args.sysroot + "/Compilers/NintendoClang/include/aarch64-nintendo-nn-elf", args.sysroot + "/Include/nn/socket", args.mono_path + "/../switch"]
